...
for n,(p1,p2) in enumerate(zip(p1_times_stack,p2_times_stack)):
    ...
    spc25_1, spc25_2 = stats.scoreatpercentile(p1, 25), stats.scoreatpercentile(p2, 25)
    spc75_1, spc75_2 = stats.scoreatpercentile(p1, 75), stats.scoreatpercentile(p2, 75)
    #print types[n],":ave1, std1, med1:", ave1, sd1, med1, " ave2, std2, med2:", ave2, sd2, med2
    print types[n],"med:", med1, med2
    print "spc1", spc25_1, spc75_1
    print "spc2", spc25_2, spc75_2
    p1_aves.append(ave1)
    p2_aves.append(ave2)
    p1_errs.append(sd1)
    p2_errs.append(sd2)
...
#p1_meds = np.array(p1_meds)
#print p1_meds.shape
ymine, ymaxe = -4, 13

fig=plt.figure()
ax = fig.add_subplot(1,2,1)
ax.boxplot(p1_times_stack)
plt.xticks([1,2,3,4], types)
plt.yticks(np.arange(ymine, ymaxe), np.arange(ymine, ymaxe))
plt.ylim(ymine, ymaxe)
plt.ylabel("time[sec]")
plt.grid()
plt.title("person switching pauses")

ax = fig.add_subplot(1,2,2)
ax.boxplot(p2_times_stack)
plt.xticks([1,2,3,4], types)
plt.yticks(np.arange(ymine, ymaxe), np.arange(ymine, ymaxe))
plt.ylim(ymine, ymaxe)
plt.ylabel("time[sec]")
plt.grid()
plt.title("robot(person') switching pauses")

#meds1 = (p1_meds[0],p1_meds[1],p1_meds[2],p1_meds[3])
#ax.boxplot(meds1)
