...
def load_persons(filenames, datalen=-1, start=0, end=1000, annobool=True, select=True):#filename is list 
    ...
    for filename in filenames:
        ...
        joints, speaks, times = json_load_data(filename, datalen, annobool) # e.g. (5000, 150),(2, 5000)
    ...
    dst_times = np.array(times_data) #(5000,)
    dict = {"joints":dst_joints, "speaks":dst_speaks, "times":dst_times}
...
def load_data_persons(filenames, datalen=-1, start=0, end=1000, annobool=True, select=True, normalize=True):#filename is list 
    ...
    for filename in filenames:
        print "Filename:", filename     
        joints, speaks, times = json_load_data(filename, datalen, annobool) # e.g. (5000, 150),(2, 5000),(5000,)
...
def json_load_data(filename, datalen, annobool):
    ...
    dst_ut = np.array(times)#(5000,)
    
    #print dst_uj.shape, dst_us.shape
    
    return dst_uj, dst_us, dst_ut


def speakNorm(data, upper=60, lower=40):
    # 上限
...
#decibel
def speakDecibelNorm(data):
    decibel = convDecibel(data)
    dst = speakNorm(decibel, upper=60, lower=40)
    return dst
...
def load_proced_data_flag(filename, datalen=-1, start=0, end=0):
    ...
    for i, fname in enumerate(filename):
        ...
        if i == 0:
            ...
            if json_file.has_key("annos"):
                ...
                annos = np.zeros((joints.shape[0], 2))
            
            if json_file.has_key("flags"): 
                flags = np.array(json_file["flags"], dtype=np.float32)
            else:
                flags = np.zeros((joints.shape[0], 1))

            #時間
            if json_file.has_key("times"): 
                times = np.array(json_file["times"])
            else:
                times = np.zeros((joints.shape[0], 1))
            ...
            if json_file.has_key("annos"):
                ...
                annos = np.vstack((annos, np.zeros((joints.shape[0], 2))))
            if json_file.has_key("flags"):
                flags = np.vstack((flags, np.array(json_file["flags"], dtype=np.float32)))
            else:
                flags = np.vstack((flags, np.zeros((joints.shape[0], 1))))

            if json_file.has_key("times"):
                times = np.vstack((times, np.array(json_file["times"])))
            else:
                times = np.vstack((times, np.zeros((joints.shape[0], 1))))
    ...
    dict = {"joints":joints, "speaks":speaks, "annos":annos, "flags":flags, "times":times}
...
# annotationの処理
#joint (1000, 36), speak (1000,1)
def proc_anno(joint, speak, use_vote=False, use_anno=True, threshold=-0.2):
    ...
    # Entry Point
    print "proc anno (use_vote:", use_vote, ", use_anno:", use_anno, ")"
    th = threshold
    ...
    z_sigs = [[joint_z_th(joint[:,i*3+2], th) for i in  idx] for idx in idxs] # Shape: e.g. (3, 2, 5000)
    ...
    user_annos = []
    for sigs, spks in zip(z_sigs, speak.T): # e.g. z_sigs:(3, 2, 5000), speak.T:(2, 5000)
        sigs = np.array(sigs)
        anns = []
        for sig, spk in zip(sigs.T, spks): # e.g. sigs.T:(5000, 2), spks:(5000,)
            if use_anno:
                ...
                ann = 1 if sig[0]+sig[1]!=0 else 0 #高さだけでAnnotationする
            anns.append(ann)
